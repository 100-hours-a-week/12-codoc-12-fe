...
export default function CustomProblemSummaryCards({ summaryCards, customProblemId, onClose }) {
  const [submitError, setSubmitError] = useState(null)
  const { sessions, updateSession, resetSession } = useCustomSummaryCardStore()
  ...
  const isSubmitEnabled = useMemo(() => {
    if (!customProblemId || summaryCards.length === 0) {
      return false
    }
  ...
  }, [customProblemId, selectedChoices, summaryCards])

  const handleSubmit = () => {
    if (!customProblemId || !isSubmitEnabled) {
      return
    }

    setSubmitError(null)

    const results = summaryCards.map((card, index) => {
      const cardKey = getCardKey(card, index)
      return selectedChoices[cardKey] === card.answerIndex
    })
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      {isGraded ? (
        <Button
          className="w-full rounded-xl bg-info text-info-foreground hover:bg-info hover:opacity-100"
          onClick={handleReset}
          type="button"
          variant="secondary"
        >
        ...
        <Button
          className={`w-full rounded-xl ${
            isSubmitEnabled ? 'bg-info text-info-foreground hover:bg-info hover:opacity-100' : ''
          }`}
